...
//two versions of vertex lighting

//version 1 (idk, i think it's right, it's the same as the Shade4PointLights thingy)
...
//version 2 (brightness increases with scale. shouldn't be an issue with normal models but with quads and stuff it is) when worldN gets normalized its the same as the one above
...
for (int index = 0; index < 4; index++){    
	float4 lightPosition = float4(unity_4LightPosX0[index], unity_4LightPosY0[index], unity_4LightPosZ0[index], 1.0);
	float3 vertexToLightSource = float3(lightPosition.xyz - worldP.xyz);        
	float3 lightDirection = normalize(vertexToLightSource);
	float squaredDistance = dot(vertexToLightSource, vertexToLightSource);
	float attenuation = 1.0 / (1.0  + unity_4LightAtten0[index] * squaredDistance);
